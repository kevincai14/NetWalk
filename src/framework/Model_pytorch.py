...
from framework.DenoisingAE_pytorch import autoencoder
...
class Model:
    ...
    def _compute_losses(self, batch_tensor):
        ...
        if num_walks == 0:
            ...
            L = torch.tensor(csgraph.laplacian(phi, normed=False), dtype=torch.float32, device=self.device)  # [walk_len, walk_len]

            # left = einsum('aij,jk->aik', t_trans_code, L)
            left = torch.einsum('aij,jk->aik', t_trans_code, L)
            # mul = einsum('aij,ajk->aik', left, trans_code)
            mul = torch.einsum('aij,ajk->aik', left, trans_code)
            # trace over last two dims for each a
            # trace_mul shape [num_walks]
            trace_mul = torch.einsum('aii->a', mul)
        ...
        for epoch in tqdm(epochs):
            ...
            if self.optimizer_type == "lbfgs":
                for batch in batches:
                    batch_tensor = torch.tensor(batch.astype(np.float32), dtype=torch.float32, device=self.device)
                    def closure():
